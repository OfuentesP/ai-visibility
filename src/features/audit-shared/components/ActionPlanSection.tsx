    ...
    <div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm leading-snug ${isTop ? 'text-slate-100 font-semibold' : 'text-slate-300 font-medium'}`}>
          ...
          <div className="px-5 py-4 space-y-3">
            {action.riesgo_inaccion ? (
              <div className="flex items-start gap-2.5">
                ...
                <p className="text-sm text-slate-300 leading-snug">{action.riesgo_inaccion}</p>
              </div>
            ) : (
                ...
                <div className="px-5 pb-5 pt-1 space-y-4 border-t border-slate-800/40">
                  ...
                  {snippet && (
                    <div className="bg-slate-950 border border-slate-800 rounded-sm overflow-hidden">
                      ...
                      <pre className="p-4 text-xs font-mono text-slate-300 overflow-x-auto leading-relaxed max-h-64 whitespace-pre"><code>{snippet}</code></pre>
                    </div>
                  )}
                </div>
              )}
        ...
        {planAccion.roi_estimado && (
          <div className="border-t border-slate-800/60 px-5 py-4">
            <div className="flex items-start gap-3 px-4 py-3 bg-emerald-950/20 border border-emerald-900/20 rounded-sm">
              ...
              <p className="text-slate-300 text-sm leading-relaxed">{planAccion.roi_estimado}</p>
            </div>
          </div>
        )}
      </motion.div>
