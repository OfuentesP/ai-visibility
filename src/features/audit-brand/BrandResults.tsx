    ...
    <motion.div ref={reportRef} id="zone-resultados" className="space-y-6" initial="hidden" animate="visible" variants={stagger}>
      ...
      <motion.div variants={{ hidden: { opacity: 0, y: -12 }, visible: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 260, damping: 24 } } }} className="grid md:grid-cols-2 gap-4">
        ...
        <div className={`border rounded-sm p-6 ${sentBg}`}>
          ...
          {d.recomendacion_ia && <p className="text-slate-300 text-sm leading-relaxed line-clamp-4">{d.recomendacion_ia}</p>}
        </div>
      </motion.div>
            ...
            <div className="flex-1">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  {accion && <p className="text-slate-300 text-sm mt-3"><span className="font-semibold">Siguiente paso:</span> {accion}{impacto ? `. ${impacto}.` : '.'}</p>}
                </div>
              </div>
            </div>
          </div>
      ...
      <motion.div id="zone-diferenciacion" variants={fadeUp} className="bg-slate-950 border border-slate-800 rounded-sm overflow-hidden">
              ...
              <tbody>
                {d.competitor_advantage.filas.map((fila, i) => (
                  <tr key={i} className="border-b border-slate-800/40 last:border-0">
                    <td className="py-3.5 pr-6 align-top"><span className="text-sm font-semibold text-slate-100">{fila.atributo_ganador}</span></td>
                    <td className="py-3.5 pr-6 align-top"><span className="text-sm text-slate-400">{fila.fuente_de_verdad}</span></td>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        {t.crecimiento_trends && (
                          <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full border ${esAlza ? 'bg-amber-500/10 text-amber-300 border-amber-500/30' : esBaja ? 'bg-rose-500/10 text-rose-300 border-rose-500/30' : 'bg-slate-800/60 text-slate-400 border-slate-700'}`}>
                            {t.crecimiento_trends}
                          </span>
                        )}
                      </div>
      ...
      {(trendsLoading || (trendsResult && trendsResult.length > 0)) && (
        <>
          ...
          <motion.div id="zone-google-trends" variants={fadeUp} className="bg-slate-900 border border-slate-800 rounded-sm p-6">
            <div className="flex items-start justify-between mb-5">
              <div>
            ...
            {trendsLoading && (
              <div className="space-y-2">
                {[0, 1, 2, 3, 4].map(i => <div key={i} className="h-8 bg-slate-800/40 rounded-sm animate-pulse" style={{ width: `${85 - i * 10}%` }} />)}
              </div>
            )}
            {!trendsLoading && trendsResult && trendsResult.length > 0 && (() => {
      ...
      {(discoveryLoading || discoveryResult) && (
        <>
          ...
          <motion.div id="zone-plan-recuperacion" variants={fadeUp} className="bg-slate-900 border border-slate-800 rounded-sm overflow-hidden">
                      ...
                      <div key={idx} className={`rounded-sm border px-4 py-3 ${op.isWin ? 'bg-emerald-950/10 border-emerald-900/20' : op.ganador ? 'bg-slate-950/40 border-rose-900/15' : 'bg-amber-950/10 border-amber-800/20'}`}>
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="shrink-0 text-right mt-0.5">
                            {op.ganador ? (
                              <span className={`text-sm font-semibold ${esMiMarca ? 'text-emerald-400' : 'text-rose-400'}`}>{esMiMarca ? `✓ ${brand}` : op.ganador}</span>
                            ) : (
            ...
            {discoveryLoading ? (
              <div className="px-5 py-4 space-y-3">{[0, 1, 2].map(i => <div key={i} className="h-12 bg-slate-800/40 rounded-sm animate-pulse" />)}</div>
      ...
      {/* Prompt de rescate */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-950 border border-slate-700 rounded-sm overflow-hidden">
