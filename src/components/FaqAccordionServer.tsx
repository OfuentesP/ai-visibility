      ...
      {items.map((item, i) => (
        <details key={i} className="group">
          <summary className="w-full flex items-start gap-5 py-5 cursor-pointer list-none">
            ...
            <span className="flex-1 text-sm font-medium text-slate-700 group-open:text-slate-900 leading-snug">
              {item.q}
            </span>
            <span className="text-slate-500 text-xs font-mono shrink-0 mt-0.5 transition-transform duration-200 group-open:rotate-45 group-open:text-indigo-600">
              +
            </span>
          </summary>
